...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            log.debug("Pas de token JWT dans la requête : {} {}", request.getMethod(), request.getRequestURI());
        ...
        try {
            ...
            if (email != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if (user != null && jwtService.isTokenValid(token, email) && user.isEnabled()) {
                    ...
                    // désactivé, ou token expiré → on laisse Spring Security gérer le 401
                    log.warn("Token rejeté : utilisateur introuvable, désactivé ou token expiré — email={}", email);
                }
            }
